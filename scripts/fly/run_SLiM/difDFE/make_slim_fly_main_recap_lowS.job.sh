...
# Make burn-in script
cat > ./scripts/temp/flyBench_recap_c${c}_${G}_h${h}_n${n}_rep${rep}_lowS.job << EOM
...
initialize() {
	...
	initializeMutationType("m2", ${h}, "g", -2.66e-6*${c}, 0.35); 	// Deleterious mutation (recessive or additive) with SelectionCoef/50
...
1:$(( ${t}/${c} + 2200000/${c} )) late() {
	...
	if (community.tick % 10000 == 0){
                writeFile("/scratch1/tferrari/SlimBenchmark/fly/logs/gen/flyBench_recap_c${c}_${G}_h${h}_n${n}_rep${rep}_lowS.gen", paste(sim.cycle));
        }
}
...
$(( ${t}/${c} + 2200000/${c} )) late() {
	...
	sim.treeSeqOutput("/scratch1/tferrari/SlimBenchmark/fly/out/burnRecap_scale${c}_gensize${G}/flyBench_recap_c${c}_${G}_h${h}_n${n}_rep${rep}_lowS.trees");
