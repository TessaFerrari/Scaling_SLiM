...
t=$((`awk 'BEGIN{FS="CYCLE "} /COALESCE/ {print $2+0}' ../../out/fly/burnCoal_scale${c}_gensize${G}/flyBench_burnin${n}_coal_c${c}_${G}.txt`*${c}))

# Make burn-in script
cat > ./temp/flyBench_coal_c${c}_${G}_h${h}_n${n}_rep${rep}.job << EOM
...
1 early() { 
	...
	sim.readFromPopulationFile("/scratch1/tferrari/SlimBenchmark/Scaling_SLiM/out/fly/burnCoal_scale${c}_gensize${G}/flyBench_burnin${n}_coal_c${c}_${G}.trees"); 
}
...
1:$(( ${t}/${c} + 2200000/${c} )) late() {
	...
	if (community.tick % 1000 == 0){
                writeFile("/scratch1/tferrari/SlimBenchmark/Scaling_SLiM/gen_logs/fly/flyBench_coal_c${c}_${G}_h${h}_n${n}_rep${rep}.gen", paste(sim.cycle));
        }
}
...
$(( ${t}/${c} + 2200000/${c} )) late() {
	...
	sim.treeSeqOutput("/scratch1/tferrari/SlimBenchmark/Scaling_SLiM/out/fly/burnCoal_scale${c}_gensize${G}/flyBench_coal_c${c}_${G}_h${h}_n${n}_rep${rep}.trees");
