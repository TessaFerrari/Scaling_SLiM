...
ZamData <- as.data.frame(read_delim("EmpiricalData/dist1000_10bins_flyEmpirical_Zambian.txt",delim="\t"))[,2:41] # Exonic data (totLength=26824075)
ZamData <- rbind(ZamData, ZamData[rep(1, 29), ])
ZamData <- cbind(multiIdx,ZamData)  
colnames(ZamData) <- c("Population", "BurnInType", "ScalingFactor", "GenomeSize", "DomCoefficent", "BurnNum", "SimNum", colnames(ZamData)[8:47])
...
ZamData$b10_sumR2 <- as.double(ZamData$b10_sumR2)

# Combine
ldData <- rbind(ldData,ZamData)
...
ldStatSummary$ScalingFactor <- factor(ldStatSummary$ScalingFactor, levels = c("Empirical","100","500","1000"))
...
afr_r2_plt = ggplot(data=ld_afr, aes(x=Bin,y=sumR2_mean,fill=ScalingFactor)) +
  ...
  scale_y_continuous(limits = c(0,0.87),expand = expansion(mult = c(0, .05)),
...
simp_afr_r2_plt <- ggplot(data = simp_ld_afr, aes(x=Bin, y=sumR2_mean, fill=ScalingFactor))  + 
  ...
  scale_y_continuous(limits = c(0,0.66),expand = expansion(mult = c(0, .05))) +
  theme(axis.text.x = element_text(angle = 50, vjust = 0.5),
...
afr_d_plt = ggplot(data=ld_afr, aes(x=Bin,y=sumD_mean,fill=ScalingFactor)) +
  ...
  ggtitle("Linkage Disequalibrium (D) of Fly African Population") +
  labs(x="Distance between SNPs (Kb)",y="Average D") + 
  scale_y_continuous(limits = c(-0.1,0.03),expand = expansion(mult = c(.05, .05))) +
  facet_nested(GenomeSize + DomCoefficent  ~ BurnInType) +
  theme(axis.text.x = element_text(angle = 50, vjust = 0.5))
...
afr_dp_plt = ggplot(data=ld_afr, aes(x=Bin,y=Dp_mean,fill=ScalingFactor)) +
  ...
  ggtitle("Linkage Disequalibrium (D') of Fly African Population") +
  labs(x="Distance between SNPs (Kb)",y="Average D'") + 
  scale_y_continuous(limits = c(-0.53,0.1),expand = expansion(mult = c(.05, .05))) +
  facet_nested(GenomeSize + DomCoefficent  ~ BurnInType) +
  theme(axis.text.x = element_text(angle = 50, vjust = 0.5))
...
# Save LD plots
ggsave(afr_r2_plt, filename = "C:/Users/tferrari/Desktop/SlimBenchmark/figures/GBE_Revisions_2/1_raw_figures/refurbished_figs/fly_african_LD_r2_wEmpirical.pdf", 
       device = cairo_pdf, width = 11, height = 4.9, units = "in")
ggsave(simp_afr_r2_plt, filename = "C:/Users/tferrari/Desktop/SlimBenchmark/figures/GBE_Revisions_2/1_raw_figures/unfaceted_main_figs/fly_african_LD_r2_wEmpirical_simple.pdf", 
       device = cairo_pdf, width = 11, height = 4.25, units = "in")
ggsave(afr_d_plt, filename = "C:/Users/tferrari/Desktop/SlimBenchmark/figures/LD/fly_african_LD_D.pdf", 
       device = cairo_pdf, width = 11, height = 8.5, units = "in")
ggsave(afr_dp_plt, filename = "C:/Users/tferrari/Desktop/SlimBenchmark/figures/LD/fly_african_LD_Dprime.pdf", 
       device = cairo_pdf, width = 11, height = 8.5, units = "in")
